...
function [fx,fy,fz,fvx,fvy,fvz,ft]=step_func_static(x,y,z,vx,vy,vz,t,t_delta,phi_0)
    ft=t+t_delta;
    ...
    K2x=((vy+t_delta/2*K1y).*midbz-(vz+t_delta/2*K1z).*midby);
    K2y=-((vx+t_delta/2*K1x).*midbz-(vz+t_delta/2*K1z).*midbx);
    K2z=((vx+t_delta/2*K1x).*midby-(vy+t_delta/2*K1y).*midbx);
    
    
    K3x=((vy+t_delta/2*K2y).*midbz-(vz+t_delta/2*K2z).*midby);
    K3y=-((vx+t_delta/2*K2x).*midbz-(vz+t_delta/2*K2z).*midbx);
    K3z=((vx+t_delta/2*K2x).*midby-(vy+t_delta/2*K2y).*midbx);
    
    K4x=((vy+t_delta*K3y).*tbz-(vz+t_delta*K3z).*tby);
    K4y=-((vx+t_delta*K3x).*tbz-(vz+t_delta*K3z).*tbx);
